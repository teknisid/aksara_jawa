...
const transliterateRoman = (to, iStr, skema, opt) => {
    ...
    if (to == "jawa") {
        //remove all space
        oStr = oStr.replace(/ +/g,'')
        //pangkon + pada lingsa -> pangkon
        oStr = oStr.replace(/꧀꧈/g,'꧀\u200C')
        //pangkon + pada lungsi -> pangkon + pada lingsa
        oStr = oStr.replace(/꧀꧉/g,'꧀꧈')
        oStr = oStr.replace(/^/gm,'꧋')
        oStr = oStr.replace(/^꧋(\s)?$/gm,'')
        // oStr = oStr.replace(/(?=[^꧀])ꦝ/g,'ꦣ')
        oStr = oStr.replace(/([^꧀])(ꦝ)/g,'$1ꦣ')
        oStr = oStr.replace(/([^꧀])(ꦝ)/g,'$1ꦣ')
    }
    if (opt) {
        if (opt.spasi == 0) {
